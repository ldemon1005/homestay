...
<script async="" defer="" src="https://maps.googleapis.com/maps/api/js?key=AIzaSyA9yWMAqS_gR49H1kcehbFpYq8V936OnBc&amp;callback=initMap">
</script>
<script type="text/javascript">
	...
	$('#room-calendar{{$bedroom->bedroom_id}}').datepicker({
		...
		...getDaysArray('{{date('Y/m/d',$book->book_from)}}','{{date('Y/m/d',$book->book_to)}}'),
		@endforeach
		]
	});
	@endforeach
				...
				<div class="row">
					<div class="col-12 col-md-12 col-lg-10">
						...
						<ul class="service">
							...
							<li><i class="fas fa-check main-color"></i> {{ $facilities->find($facility)->facility_name ?? "Null"}}</li>
							@endforeach
						</ul>
					</div>
				</div>
				...
				<div class="room-item room_{{$bedroom->bedroom_id}}">
					...
					<div class="room-content">
						...
						<ul class="room-service">
							<li><i class="fas fa-bath"></i> @if($bedroom->bedroom_bath == 1) Khép kín @else Chung @endif</li>
							<li><i class="fas fa-user"></i> {{ $bedroom->bedroom_slot }}</li>
							<li><i class="fas fa-bed"></i> 2</li>
						</ul>
						<p class="fs-14 bold uppercase">Giá: {{ number_format($bedroom->bedroom_price) }} Đ/ĐÊM</p>
						...
						<a style="cursor: pointer" onclick="add_order({{$bedroom->bedroom_id}},{{$bedroom->bedroom_price}})" class="book-btn
						hs-btn-90-30
						hs-btn-green">Đặt
							phòng</a>
					</div>
					...
					<div class="collapse" id="collapseExample{{$bedroom->bedroom_id}}">
						<div class="room-detail">
							...
							<ul class="mb-4">
								...
								<li><i class="fas fa-check main-color"></i> {{$bedroom_facilities->find($facility)->bedroom_facility_name ?? 'Không có tiện ích'}}</li>
								@endforeach
							</ul>
			...
			<div class="col-12">
				...
				<div class="review-item">
					<div class="review-user">
						...
						<p class="fs-10 grey-9 mb-0 center"><i class="fas fa-map-marker"></i> {{$comment->user->nation}}</p>
					...
					<div class="review-content">
						<div>
							...
							@if($comment->comment_rate >= 10) Rất tốt @elseif($comment->comment_rate >= 8) Tốt @elseif($comment->comment_rate >= 6) Bình thường @elseif($comment->comment_rate >= 4) Không tốt @else($comment->comment_rate >= 2) Rất không tốt @endif
						</div>
